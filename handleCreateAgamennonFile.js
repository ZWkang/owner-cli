...
if(isOwnerrcExist) {
    ...
    const { rootPath = defaultRootPath, components = defaultComponentPositions, docs = defaultDocsFilePositions } = Agamemnon
    excaRoot = rootPath
...
const indexFileContent = name => `import ${firsrWordUpper(name)} from './${firsrWordUpper(name)}';
...
function main(name) {
    ...
    const componentfile = path.join(excaRoot, excaComponents, name, firsrWordUpper(name) + '.js')
